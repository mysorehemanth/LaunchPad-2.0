...
begin
       //GUI display
       CMDedit.Text := Concat(GetCurrentDir,'\PK3_Pack\PK3CMD -P24FJ64GA004 -C');
...
begin
       ...
       //GUI display
       CMDedit.Text := Concat(GetCurrentDir,'\PK3_Pack\PK3CMD -P24FJ64GA004 -F',HexFileName,' -Y');
       Label2.Visible :=False;
       Label1.Visible := False;
...
begin
       //GUI display
       CMDedit.Text := Concat(GetCurrentDir,'\PK3_Pack\PK3CMD -P24FJ64GA004 -L');
       Label2.Visible := False;
       Label1.Visible := False;
...
begin
       ...
       //GUI display
       CMDedit.Text := Concat(GetCurrentDir,'\PK3_Pack\PK3CMD -P24FJ64GA004 -F',HexFileName,' -M');
       Label2.Visible := False;
       Label1.Visible := False;
       Label3.Caption := 'Programming...';
       //execute
       IndicatorController( RunDosInMemo(Concat(CMDedit.Text,HexFileName,'" -M'),Memo1,'Programming/Verify complete','Failed to' ));
end;

procedure TForm1.EraseButtonClick(Sender: TObject);
begin
       //GUI display
       CMDedit.Text := Concat(GetCurrentDir,'\PK3_Pack\PK3CMD -P24FJ64GA004 -E');
       Label2.Visible := False;
       Label1.Visible := False;
...
begin
   ...
   InsertHexNumber(MID_FILE_NAME,FINAL_FILE_NAME,SNLineNo,SerialNumber,DEFAULT_EMPTY_SN);

   //Modify Producting date
   ProductDate := DateToStr(Date);
   ProductDate := StringReplace(ProductDate,'/','',[rfReplaceAll]);
   Label9.Caption := ProductDate;
   PDLineNo := FindLineLocation(HexFileName,DEFAULT_EMPTY_PD);
   InsertHexNumber(HexFileName,MID_FILE_NAME,PDLineNo,ProductDate,DEFAULT_EMPTY_PD);
...
{procedure TForm1.Edit2KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (not(Key in ['0'..'9']) and not(Key = #8) and not(Key = #13)) then Edit2.Text :='';
end;}
...
while count <=4 do
  ...
  Case count of
    ...
    3: begin
       CMDedit.Text := Concat(GetCurrentDir,'\PK3_Pack\PK3CMD -P24FJ64GA004 -F"',FINAL_FILE_NAME,'" -M');
